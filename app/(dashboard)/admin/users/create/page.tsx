...
export default function CreateUserPage() {
  ...
  const { locations, availableLocations, roles, addUser } = useAppData();
  ...
  React.useEffect(() => {
    if (!formData.roleId && roleOptions.length) {
      const sales = roleOptions.find((r) => r.name === "Sales") || roleOptions[0];
      setFormData((prev) => ({ ...prev, roleId: sales.id, role: sales.name }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addUser({ ...formData, role: selectedRole?.name || formData.role });
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 animate-in fade-in duration-500">
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-xl space-y-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 md:p-8">
        ...
        <div className="space-y-1.5">
          <label className="px-1 text-[11px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-300">Password</label>
